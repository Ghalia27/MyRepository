...
btn_registrar.addEventListener("click", function () {
  ...
  datosUsuarios.append("id_usuario", id_usuario); //nombre, apellido y cel en un objeto
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState == 4) {
      if (xhr.status == 200) {
        ...
        console.log(resultado); //arreglo de objetos

        listado_personas.unshift(usuarioJson);
        let html = dibujar(listado_personas);
        listado.innerHTML = html; //listar();
      }
    }
  };
}); //buscar 
...
function pagar(nombre, apellido, telefono) {
  $("#exampleModal").modal("show"); //console.log(nombre,apellido,telefono)
...
registrar.addEventListener("click", function () {
  ...
  let fecha = Date.now(); //console.log(nombre,telefono,pago,fecha);
  ...
  fetch(url, config).then(data => {
    ...
    console.log('pago', data); //oculta modal
